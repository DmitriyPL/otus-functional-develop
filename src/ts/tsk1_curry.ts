...
export type MyCurry<P extends any[], R> = (
    arg0: Head<P>
  ) => HasTail<P> extends true ? MyCurry<Tail<P>, R> : R;
  
declare function CurryV0<P extends any[], R>( f: (...args: P) => R ): MyCurry<P, R>;
    
export function task1(){

    const func = (a: number, b: number, c: number, d: number, e: number) => a + b + c + d + e;
    const hof = CurryV0(func);
